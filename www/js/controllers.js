...
.controller('gallery', function($scope,$http) {
    ...
    db.transaction(function(tx){
        ...
        //tx.executeSql('CREATE TABLE IF NOT EXISTS album (galId INTEGER, businessId TEXT,galName TEXT,status TEXT)');
        
        //for(var i = 0; i < business.rows.length; i++) {
        //tx.executeSql('INSERT INTO album(galId, businessId,galName,status) VALUES ("1","1","menu","ON")',successID);
        //tx.executeSql('INSERT INTO album(galId, businessId,galName,status) VALUES ("2","1","item","ON")',successID);
        //}
        
        
        //tx.executeSql('SELECT g.*, a.galName  FROM  gallery g INNER JOIN album a ON g.galId= a.galId where g.galId="1"',[], function (tx, results){
        //    //SELECT * FROM table_a a INNER JOIN table_b b ON a.id=b.other_id WHERE b.property_id=?
        //    for(var i=0;i<results.rows.length;i++){
        //        alert(results.rows.item(i).galId+" gal id");
        ...
        tx.executeSql('SELECT * FROM  album where status="ON"',[], function (tx, results){
            ...
            //alert(results.rows.length=" album length ");
                    for(var j=0; j < results.rows.length; j++)
                    {
                        ...
                        //alert('<h3 class="">'+results.rows.item(j).galId+'"'+results.rows.item(j).galName+'</h3>');
                        
                        //where galId="'+albumData.item(j).galId+'"
                        //alert(results.rows.item(j).galId);
                        ...
                        tx.executeSql('SELECT g.*, a.galName  FROM  gallery g INNER JOIN album a ON g.galId= a.galId where g.galId="'+row.galId+'"',[], function (tx, res){
                           // alert("dayte");
                        //alert(res.rows.item(0).galId +" gllery");
                        //alert(res.rows.item(0).galName +" gllery");
                        // $('#lightbox').append('<h3 class="">'+results.rows.item(j).galName+'</h3>');
                      ...
                      //albumJsonArr.push(row);
                    }
                //$scope.$apply(function() {
                //    $scope.albumData=albumJsonArr;
                //});
        });
        ...
        //tx.executeSql('SELECT * FROM  album where status="ON"',[], function (tx, results){
        //    //var itemLength = results.rows.length;
        //           // var albumData=results.rows;
        //            
        //            alert(results.rows.length=" album length ");
        //            //for(var j = 0; j < results.rows.length; j++)
        //            //{
        //            //    var row = albumData.item(j);
        //            //    $('#lightbox').append('<h3 class="">'+results.rows.item(j).galName+'</h3>');
        //            //    //where galId="'+albumData.item(j).galId+'"
        //            //    tx.executeSql('SELECT * FROM  gallery where galId="'+albumData.item(j).galId+'"',[], function (tx, res){
        //            //      alert(res.rows.item(i).galId +" gllery");
        //            //        for(var i=0;i<res.rows.length;i++){      
        //            //            $('#lightbox').append('<div class="col-md-3 col-sm-4 col-xs-6" data-src='+ res.rows.item(i).images +' ><div class="lightbox-item p-item"> <img alt="images" src='+ res.rows.item(i).images + ' class="animated flip"></div></div>'); 
        //            //            if(i >= res.rows.length-1){
        //            //                $('.lightbox').lightGallery({
        ...
        //    
        //    for(var i=0;i<results.rows.length;i++){
        //        alert(results.rows.item(i).galId+" gal Id from Gallery");
        //        //$('#lightbox').append('<div class="col-md-3 col-sm-4 col-xs-6" data-src='+ results.rows.item(i).images +' ><div class="lightbox-item p-item"> <img alt="images" src='+ results.rows.item(i).images + ' class="animated flip"></div></div>'); 
        //        //if(i >= results.rows.length-1){
        //        //    $('.lightbox').lightGallery({
        //        //        enableTouch: true
